...
const DynamicCompHoverColor = ({ section, title }) => {
  ...
  const [hoverBackgroundColor, setHoverBackgroundColor] = useState("#f0f0f0");
  ...
  useEffect(() => {
    if (selectedColorControl) {
      setBackgroundColor(selectedColorControl.backgroundColor || "#ffffff");
      setTextColor(selectedColorControl.textColor || "#000000");
      setHoverBackgroundColor(selectedColorControl.hoverBackgroundColor || "#f0f0f0");
      setHoverTextColor(selectedColorControl.hoverTextColor || "#000000");
      setFontSize(selectedColorControl.fontSize || 16);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedColorControl) {
      ...
      if (result?.error) {
        ...
        setTextColor("#000000");
        setHoverBackgroundColor("#f0f0f0");
        setHoverTextColor("#000000");
        setFontSize(16);
      }
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <div className="space-y-2">
          <label
            ...
            className="block text-sm font-medium text-gray-700"
          >
            Hover Background Color (Type or Pick):
          </label>
          <div className="flex gap-2">
            <input
              type="text"
              ...
              placeholder="Enter hover background color (e.g., #f0f0f0, lightblue)"
              className="w-full px-3 py-1 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-yellow-600 disabled:cursor-not-allowed"
              disabled={selectedColorControl && !isEditing}
            />
            <input
              type="color"
              id="hoverBackgroundColorPicker"
              value={hoverBackgroundColor}
              onChange={(e) => setHoverBackgroundColor(e.target.value)}
              className="w-12 h-8 p-1 border border-gray-300 rounded disabled:cursor-not-allowed"
              disabled={selectedColorControl && !isEditing}
        ...
        <div className="space-y-2">
          <label
            htmlFor="hoverTextColor"
            className="block text-sm font-medium text-gray-700"
          >
            Hover Text Color (Type or Pick):
          </label>
          <div className="flex gap-2">
            <input
              type="text"
              ...
              placeholder="Enter hover text color (e.g., #000000, darkblue)"
              className="w-full px-3 py-1 border border-gray-300 focus:outline-none rounded focus:ring-1 focus:ring-yellow-600 disabled:cursor-not-allowed"
              disabled={selectedColorControl && !isEditing}
            />
            <input
              type="color"
              id="hoverTextColorPicker"
              value={hoverTextColor}
              onChange={(e) => setHoverTextColor(e.target.value)}
              className="w-12 h-8 p-1 border border-gray-300 rounded disabled:cursor-not-allowed"
              disabled={selectedColorControl && !isEditing}
...
export default DynamicCompHoverColor;